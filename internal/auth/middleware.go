...
func RequireRole(min Role) tele.MiddlewareFunc {
	return func(next tele.HandlerFunc) tele.HandlerFunc {
		return func(c tele.Context) error {
			sender := c.Sender()
			if sender == nil || !Has(sender.ID, min) {
...
// RequirePermission requires an explicit business permission. Admin and owner
// roles satisfy every business permission through HasPermission.
func RequirePermission(permission Permission) tele.MiddlewareFunc {
	return func(next tele.HandlerFunc) tele.HandlerFunc {
		return func(c tele.Context) error {
			sender := c.Sender()
			if sender == nil || !HasPermission(sender.ID, permission) {
				return c.Send(fmt.Sprintf("⛔ 需要 %s 权限", permission))
			}
			return next(c)
		}
	}
